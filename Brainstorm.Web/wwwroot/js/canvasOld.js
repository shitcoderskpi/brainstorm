...
function initCanvas() {
    ...
    canvas.setAttribute('width', innerWidth * 0.98);
    canvas.setAttribute('height', innerHeight * 0.95);
}
...
function mouseDown(e) {
    if (e.button === 0) {
        ...
        prevPoint = getMouse(e, canvas); // get mouse cords
        isMoving = true;
    }
}
...
function mouseMove(e) {
    if (isDrawing)
    {
        ...
        offsetX += mouseCoords.x - prevPoint.x;
        offsetY += mouseCoords.y - prevPoint.y;
        prevPoint = mouseCoords;
        tiledCanvas.goto(canvasPosition.x, canvasPosition.y);
...
function redrawCanvas() {
    ...
    ctx.drawImage(memCanvas, 0, 0);                               // Draw memory lines
...
function drawPoints(ctx, points) {
    ...
    for (let i = 1; i < n - 2; i++) { // point length - 2 because we are already on first and after loop we will move to the last one
    ...
    tiledCanvas.drawingRegion(points[0].x, points[0].y, points[n - 1].x, points[n - 1].y);
    tiledCanvas.execute();
}
